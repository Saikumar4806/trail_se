...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (itemForm) {
      ...
      const fetchItems = async () => {
        try {
          ...
          if (response.ok && result.success) {
            ...
            itemsTableBody.innerHTML = '<tr><td colspan="6" class="text-center error-msg">Failed to load items.</td></tr>';
          }
        } catch (err) {
          console.error("Error fetching items:", err);
          itemsTableBody.innerHTML = '<tr><td colspan="6" class="text-center error-msg">Cannot connect to server.</td></tr>';
        }
      };
      ...
      const renderItems = (items) => {
        if (items.length === 0) {
          itemsTableBody.innerHTML = '<tr><td colspan="6" class="text-center">No items found.</td></tr>';
          return;
        }
        ...
        itemsTableBody.innerHTML = items.map(item => `
          <tr>
            ...
            <td>${item.quantity}</td>
            <td>
              <button class="btn-edit btn-sm" onclick="editItem(${item.id}, '${item.name.replace(/'/g, "\\'")}', ${item.price}, '${item.category || ''}', ${item.quantity}, '${item.image_url || ''}')">Edit</button>
      ...
      window.editItem = (id, name, price, category, quantity, imageUrl) => {
